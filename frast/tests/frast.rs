...
fn main() {
    ...
    println!("\nKeystream bits");
    for (i, he_bits_list) in he_keystream_bits.chunks_exact(MODULUS_BIT).enumerate() {
        for (bit_idx, he_bit) in he_bits_list.iter().enumerate() {
            let correct_val = (state[i] & (1 << bit_idx)) >> bit_idx;
            ...
            let keystream_bit_idx = 4 * i + bit_idx;
            println!("keystream[{keystream_bit_idx:>3}]: {correct_val:x} | he_keystream[{keystream_bit_idx:>3}]: {decoded:x} | bit_error: {err:>2}");
    ...
    for (mut he_out, he_bits) in he_out_list.iter_mut().zip(he_keystream_bits.chunks_exact(2)) {
        ...
        for i in 0..2 {
            ...
            lwe_ciphertext_add_assign(&mut buf, &he_pbs);
        }

        let now = Instant::now();
        keyswitch_lwe_ciphertext(&ksk_from_online, &buf, &mut he_out);
        time_online_ks += now.elapsed();
    }

    for (i, he_out) in he_out_list.iter().enumerate() {
        let correct_val = if i % 2 == 0 {
            state[i / 2] & 0x3
        } else {
            (state[i / 2] & 0xc) >> 2
        };
